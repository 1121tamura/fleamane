CREATE TABLE app_settings (
    ...
    key         TEXT NOT NULL UNIQUE,    -- 設定キー（例: service.mercari.enabled）
    value       TEXT NOT NULL,           -- 設定値（JSON or 文字列）
    category    TEXT NOT NULL,           -- 'service'|'external_api'|'accounting'|'app'
    description TEXT,                    -- 画面表示用の説明
...
CREATE TABLE listings (
    ...
    platform_item_id    TEXT NOT NULL,   -- プラットフォーム上の商品ID
    platform            TEXT NOT NULL,   -- 'mercari' | 'yahooauction'
    title               TEXT NOT NULL,
    description         TEXT,
    category            TEXT,
...
CREATE TABLE trades (
    ...
    platform            TEXT NOT NULL,    -- 'mercari' | 'yahooauction'
    status              TEXT NOT NULL,    -- 'purchased'|'waiting_shipment'|'shipped'|'completed'|'cancelled'
...
CREATE TABLE price_history (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    listing_id INTEGER NOT NULL REFERENCES listings(id),
    old_price  INTEGER NOT NULL,           -- 変更前価格（円）
    new_price  INTEGER NOT NULL,           -- 変更後価格（円）
...
CREATE TABLE system_logs (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    service    TEXT NOT NULL,     -- サービス名（例: mercari, launcher）
    level      TEXT NOT NULL,     -- 'info'|'warn'|'error'
    message    TEXT NOT NULL,
    detail     TEXT,              -- JSON形式の詳細情報
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
